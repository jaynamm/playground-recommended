...
def post_recommended_data(input_data):
    ...
    # 유사도 검증
    skill_df['similarity_score'] = skill_df.apply(lambda row: sum(0 if (row[column] > user_profile[column]).all() else (int(row[column]) / int(user_profile[column])) for column in skill_df.columns[1:]), axis=1)
